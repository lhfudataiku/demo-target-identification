...
#           enriched_gene_druggability/_safety  (upstream annotations, never model inputs)
...
sc = sc.dropna(subset=["proba_1", "gene_ppi_degree"])
pos = sc[sc.is_target == 1].copy()
pos["q"] = pd.qcut(pos.gene_ppi_degree, 5, labels=False, duplicates="drop")

print("HUBHDR|quintile|median_degree|mean_score|predicted_positive_pct|n")
band = {}
for q, g in pos.groupby("q"):
    band[int(q)] = (float(g.gene_ppi_degree.median()), float(g.proba_1.mean()),
                    100.0 * float((g.proba_1 >= 0.5).mean()), len(g))
...
# The mechanism we tested and REFUTED — module size does not explain the champion's gain.
rho = float(pd.Series(sc.gene_ppi_degree).rank().corr(pd.Series(sc.proba_1).rank()))
...
# ============================================================================
raw = dataiku.Dataset("raw_ot_known_drug").get_dataframe()
cols = {c.lower(): c for c in raw.columns}
drug_c = cols.get("drugid") or cols.get("drug_id") or list(raw.columns)[0]
targ_c = cols.get("targetid") or cols.get("target_id")
dis_c = cols.get("diseaseid") or cols.get("disease_id")
if targ_c and dis_c:
    per_drug = raw.groupby(drug_c)[targ_c].nunique()
    multi = per_drug[per_drug > 1]
    pairs_from_multi = raw[raw[drug_c].isin(multi.index)]
    pct_multi = 100.0 * len(pairs_from_multi) / max(len(raw), 1)
    single = per_drug[per_drug == 1]
    pct_single = 100.0 * len(raw[raw[drug_c].isin(single.index)]) / max(len(raw), 1)
    print(f"TRUTH|drugs={len(per_drug):,} | multi-target drugs={len(multi):,} "
          f"({100 * len(multi) / len(per_drug):.1f}%) | pairs from multi-target={pct_multi:.1f}% "
          f"| pairs surviving a single-target demand={pct_single:.1f}%")
    check("5.4 pairs from multi-target drugs %", 82.0, round(pct_multi, 0), tol=3.0, fmt="{:.0f}")
    check("5.4 pairs surviving single-target %", 8.0, round(pct_single, 0), tol=3.0, fmt="{:.0f}")
    worst = per_drug.max()
    print(f"TRUTH|worst single drug hits {worst} targets — every one of them becomes a 'validated' pair")
else:
    print("TRUTH|SKIPPED — raw_ot_known_drug column names not recognised; inspect and fix the mapping")
...
try:
    dg = dataiku.Dataset("enriched_gene_druggability").get_dataframe()
    print(f"XCHECK|enriched_gene_druggability rows={len(dg):,} "
