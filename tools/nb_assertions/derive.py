"""Recompute zone-90 evidence tables in code, instead of reading a recipe's output.

THE NOTEBOOK RULE THIS IMPLEMENTS. "Read the most upstream dataset that still carries the number and
recompute in code. Reading a derived table and asserting its contents proves the recipe still runs,
not that the number is right." Zone 90 existed as staging for exactly this conversion.

WHY A SHARED MODULE AND NOT A COPY IN EACH SCRIPT. `tractability_axis` is read by BOTH nb4 (section
8.4) and nb6 (acts 5-6). A copy per script would recreate the duplication that
`tools/push_assertions.py` was written to remove -- the assertion logic used to exist in three
places and `.index/assertions.tsv` described the copy that was not being run.

HOW THESE FUNCTIONS WERE PRODUCED. Each body is the corresponding `dss_recipes/` mirror, wrapped in
a function with exactly two changes: the terminal `write_with_schema` is removed and the frame is
returned instead, and `print` is routed through `_p` so a caller can silence the diagnostics. The
logic is not retyped, so it cannot drift from the recipe it replaces during the migration itself.
Once the recipe is deleted this module becomes the only copy, and `tools/check_indexes.sh` gates it
against the DSS library copy.
"""
...
def _mk_printer(verbose):
    return print if verbose else (lambda *a, **k: None)


# ---------------------------------------------------------------------------
# Design rationale, carried over verbatim from the retired compute_tractability_axis
# recipe. It is the reason the degree-matched column exists and must not be dropped
...
# ---------------------------------------------------------------------------
def tractability_axis(verbose=True):
    """Axis 2: are the model's top-ranked candidates actionable?

    Returns the frame that the retired `compute_tractability_axis` recipe wrote to the
    `tractability_axis` dataset: one row per (disease, scope) with naive and
    degree-matched lift at each K. The degree-matched column is the honest one -- naive
    lift partly measures gene popularity, and the gap between them is the act-6 punch line.

    Reads `graph_nodes`, `drug_protein_edges`, `enriched_gene_druggability_v2` and a narrow
    5-column slice of `scored_champion`.
    """
    _p = _mk_printer(verbose)
    ...
    _p(f"genes with a demonstrated drug-target edge: {len(demo_genes):,} of {len(G):,} "
    ...
    _p(f"genes with an assessed tractability bucket : {int(drg.assessed.sum()):,} "
    ...
    for c in ["demonstrated", "assessed"]:
        _p(f"  {c:14s}" + "  ".join(f"Q{q+1} {100*qrate[c][q]:.1f}%" for q in sorted(qrate[c])))
    ...
    for scope in ["all candidates", "novel only"]:
        ...
        _p(f"\n=== {scope.upper()} ===")
        _p(f"  {'label':14s}{'K':>5s}{'obs':>8s}{'exp(dm)':>9s}{'naive lift':>12s}{'DEGREE-MATCHED':>16s}")
        for lab in ["demonstrated", "assessed"]:
            for K in KS:
                o = int(s[f"{lab}_obs{K}"].sum()); e = s[f"{lab}_exp{K}"].sum()
                _p(f"  {lab:14s}{K:>5d}{o:>8,}{e:>9,.0f}"
    ...
    _p(f"  degree-matched DEMONSTRATED tractability lift, novel head: "
          f"top20 {dm20:.2f}x, top50 {dm50:.2f}x")
    if dm20 <= 1.05:
        _p("  CONFIRMED -- at or below parity. The model does NOT preferentially rank actionable")
        _p("  genes, consistent with the ligand-vs-receptor failure being real and general.")
    else:
        _p("  REFUTED -- the head IS enriched for actionable genes beyond hub effects, so the")
        _p("  ligand-vs-receptor problem is narrower than §8.7 claims and should be softened.")

    _p("\n=== composition of the top 50 vs the candidate pool (novel only) ===")
    _p(f"  secreted   top50 {nov.pct_secreted_top50.mean():.1f}%  vs pool {nov.pct_secreted_all.mean():.1f}%")
    _p(f"  membrane   top50 {nov.pct_membrane_top50.mean():.1f}%  vs pool {nov.pct_membrane_all.mean():.1f}%")
    _p("  Secreted over-represented at the head while membrane is under-represented is the")
    _p("  ligand-vs-receptor failure stated as a distribution rather than an anecdote.")
